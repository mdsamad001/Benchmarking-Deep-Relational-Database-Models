...
class HeteroDataBuilder:
    def __init__(
        ...
        if df_converter is None:
            df_converter = self.extend_default_df_converter(
                schema=schema, target=(self.target_table, self.target_column)
            )
    ...
    @staticmethod
    def extend_default_df_converter(
        *converters: Tuple[Optional[Type[ColumnDef]], SeriesConverter],
        schema: Schema,
        target: Tuple[str, str],
        target_converters: Optional[
            List[Tuple[Optional[Type[ColumnDef]], SeriesConverter]]
        ] = None,
    ) -> DataFrameConverter:
        the_converters: OrderedDict[Optional[Type[ColumnDef]], SeriesConverter] = (
            OrderedDict(
                (
        ...
        the_target_converters: OrderedDict[Optional[Type[ColumnDef]], SeriesConverter] = (
            OrderedDict(
                (
                    (CategoricalColumnDef, CategoricalConverter()),
                    (NumericColumnDef, IdentityConverter()),
    ...
    def _table_to_dataframe_raw(self, table_name: str) -> pd.DataFrame:
        ...
        df = pd.read_sql(con=self.db_inspector.connection, sql=table_name)

        return df

    def _fk_to_index(
        self, fk_def: ForeignKeyDef, table: pd.DataFrame, ref_table: pd.DataFrame
    ) -> torch.Tensor:
    ...
    def _convert_dataframes_and_target(
        self, table_dfs: Dict[str, pd.DataFrame]
    ) -> Tuple[Tuple[pd.DataFrame, Dict[str, ColumnDef]], Dict[str, Dict[str, ColumnDef]]]:
        ...
        if len(target_df.columns) != 1:
            warnings.warn(
                f"The original target {self.target_table}.{self.target_column} "
    ...
    @overload
    def build_as_pandas(
        self, with_target: Literal[True]
    ) -> Tuple[Dict[str, pd.DataFrame], pd.DataFrame]: ...

    @overload
    def build_as_pandas(
        self, with_target: Literal[False] = False
    ) -> Dict[str, pd.DataFrame]: ...

    def build_as_pandas(
        self, with_target: bool = False
    ) -> Union[Dict[str, pd.DataFrame], Tuple[Dict[str, pd.DataFrame], pd.DataFrame]]:
    ...
    @overload
    def build(
        self, with_column_names: Literal[True]
    ) -> Tuple[HeteroData, Dict[str, List[ColumnDef]], Dict[str, List[str]]]: ...

    @overload
    def build(
        self, with_column_names: Literal[False] = False
    ) -> Tuple[HeteroData, Dict[str, List[ColumnDef]]]: ...

    def build(self, with_column_names: bool = False) -> Union[
        Tuple[HeteroData, Dict[str, List[ColumnDef]]],
        Tuple[HeteroData, Dict[str, List[ColumnDef]], Dict[str, List[str]]],
    ]:
        out = HeteroData()
        ...
        for table_name, table_schema in self.schema.items():
            for fk_def in table_schema.foreign_keys:
                ...
                try:
                    out[id].edge_index = self._fk_to_index(
                        fk_def, table_dfs[table_name], table_dfs[fk_def.ref_table]
                    )
        ...
        if y.shape[-1] > 1:
            raise ValueError(
                f"Target ({self.target_table}.{self.target_column}) shouldn't have more than 1 feature. "
            ...
            raise ValueError(
                f"Target ({self.target_column}) has no values after conversion. "
        ...
        for table_name, df in table_dfs.items():
            out[table_name].x = torch.from_numpy(df.to_numpy(dtype=np.float32)).to(
                self.device
            )
            this_labels = [str(col) for col in df.columns]
            out_column_defs[table_name] = [
                column_defs[table_name][column_name] for column_name in this_labels
            ]
