...
class BlueprintModel(torch.nn.Module):
    def __init__(
        ...
        use_griffin_embedder: bool = False,
        griffin_device: str = 'cuda',
        griffin_pretrained_float_path: Optional[str] = None,
        griffin_table_dataframes: Optional[Dict[NodeType, "pd.DataFrame"]] = None
    ):
        super().__init__()
        ...
        if use_griffin_embedder:
            print("###############################################Using Griffin")
            try:
                from db_transformer.nn.embedder.griffin_db_embedder import GriffinDBEmbedder

                self.embedder = GriffinDBEmbedder(
                    embed_dim=embed_dim,
                    col_stats_dict=col_stats_per_table,
                    col_names_dict_per_table=col_names_dict_per_table,
                    device=griffin_device,
                    pretrained_float_path=griffin_pretrained_float_path,
                    table_dataframes=griffin_table_dataframes
                )
                print("✓ Using Griffin embedder (Nomic + pre-trained MLP)")
            except ImportError as e:
                print(f"WARNING: Failed to import GriffinDBEmbedder: {e}")
                print("Falling back to standard DBEmbedder")
                use_griffin_embedder = False

        if not use_griffin_embedder:
            from db_transformer.nn import DBEmbedder
            ...
            print("✓ Using standard DBEmbedder")
    ...
    def forward(
        ...
        if self.target_table not in x_dict and self.target_table in tf_dict:
            print(
                f"[BlueprintModel] WARNING: target table '{self.target_table}' "
                f"missing in x_dict; embedding target table separately."
            )

            x_target_only = self.embedder({self.target_table: tf_dict[self.target_table]})
            if self.target_table in x_target_only:
                x_dict[self.target_table] = x_target_only[self.target_table]
            else:
                raise KeyError(
                    f"Target table '{self.target_table}' missing after fallback embedding. "
                    f"x_target_only keys: {list(x_target_only.keys())}"
                )


        if self.target_table not in x_dict:
            raise KeyError(
                f"Target table '{self.target_table}' not found in x_dict. "
    ...
    def _remove_empty(
        ...
        for node, tf in tf_dict.items():
            ...
            if tf.num_rows > 0 or node == self.target_table:
                out_tf_dict[node] = tf
