...
class DBGNNLayer(torch.nn.Module):
    def __init__(self, dim, metadata, aggr, schema):
        ...
        convs = {
            m: GATv2Conv(-1, dim, aggr=aggr, add_self_loops=False) for m in metadata[1]
        }
...
class DBGNN(torch.nn.Module):
    def __init__(
        ...
        self.gnn_layers = torch.nn.ModuleList(
            [DBGNNLayer(proj_dim, metadata, aggr, schema) for _ in range(layers)]
        )
    ...
    def forward(self, x_dict, edge_index_dict):
        ...
        x_dict = {
            k: self.table_projection[k](x.view(*x.shape[:-2], -1))
