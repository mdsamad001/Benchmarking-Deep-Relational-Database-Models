...
class TextEmbedder:
    def __init__(self, batch_size, embedding_model, device_type):
        self.model = SentenceTransformer(
            ...
            device=device_type,
            model_kwargs={"dtype": torch.bfloat16 if device_type == "cuda" else torch.float32},
        )
...
def main(dataset_name, device=None, batch_size=8192, embedding_model="all-MiniLM-L12-v2"):
    ...
    device_type = torch.device(device[0] if isinstance(device, list) else device).type

    text_path = f"{os.environ['HOME']}/scratch/pre/{dataset_name}/text.json"
    with open(text_path) as f:
        text_list = orjson.loads(f.read())

    embedder = TextEmbedder(batch_size, embedding_model=embedding_model, device_type=device_type)
    emb_list = embedder(text_list, device=device)
